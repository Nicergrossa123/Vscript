...
using Nexus.Module.Paintball.Menu;
using System.Xml;
using System.Linq;
        ...
        public override bool OnKeyPressed(DbPlayer dbPlayer, Key key)
        {
            if (dbPlayer.Dimension[0] == 0 && key == Key.E)
            {
                if (dbPlayer.Player.Position.DistanceTo(PaintballMenuPosition) < 2.0f)
                {
                    ...
                    Module.Menu.MenuManager.Instance.Build(Nexus.Module.Menu.PlayerMenu.PaintballEnterMenu, dbPlayer).Show(dbPlayer);
                    return true;
                }
            }
                return false;
                        ...
                        if (iPlayer != iKiller)
                        {
                            ...
                            if (pba.pbPlayers.ContainsKey(iKiller))
                            {
                                ...
                                if (pba.pbPlayers[iKiller].killstreak == 12)
                                {
                                    foreach (var Players in pba.pbPlayers)
                                    {
                                        Players.Key.Player.TriggerEvent("sendGlobalNotification", $"{iKiller.Player.Name} - Rezq_Blxck, bist du es?", 5000, "white", "glob");
                                    }
                                }
