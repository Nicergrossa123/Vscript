...
namespace Nexus.Module.Injury
{
    public sealed class InjuryModule : Module<InjuryModule>
    {
        ...
        public static Vector3 InsurancePos = new Vector3(350.19, -590.155, 28.79);
        ...
        public override bool OnColShapeEvent(DbPlayer dbPlayer, ColShape colShape, ColShapeState colShapeState)
        {
            if (colShapeState == ColShapeState.Enter && colShape.HasData("injuryDeliverId"))
            {
                if (dbPlayer.isInjured() && dbPlayer.Player.IsInVehicle)
                {
                    ...
                    DbPlayer medic = sxVehicle.GetOccupants().Values.Where(p => p.Player.VehicleSeat == -1).FirstOrDefault();
