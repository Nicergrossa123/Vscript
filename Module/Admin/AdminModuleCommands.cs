...
using Nexus.Module.Players.Ranks;
using Nexus.Module.Spawners;
using Nexus.Module.Teams;
using Nexus.Module.Teams.Permission;
...
using static Nexus.AnimationContent;
using Nexus.Module.Node;
...
using Nexus.Module.Computer.Apps.FahrzeuguebersichtApp.Apps;
using Nexus.Module.Computer.Apps.FahrzeugUebersichtApp;
using Nexus.Module.Computer.Apps.KennzeichenUebersichtApp;
using Nexus.Module.Computer.Apps.KennzeichenUebersichtApp.Apps;
using Configuration = Nexus.Module.Configurations.Configuration;
using Nexus.Module.Zone;
using Nexus.Module.Players.Commands;
using static Nexus.Module.Players.PlayerNotification;
using Nexus.Module.Business;
using System.Net;
using Nexus.Module.Helper;
using Nexus.Module.PlayerName;
using Nexus.Module.Attachments;
using Nexus.Module.Dealer;
using Nexus.Module.Kasino;
using Nexus.Module.Clothes;
using Nexus.Module.Configurations;
using Nexus.Module.Vehicles.Fuel;
using Nexus.Module.Laboratories;
using Nexus.Module.Guenther;
...
using Nexus.Module.Space;
using System.Diagnostics;
using Nexus.Module.Events;
using Nexus.Module.Weather;

namespace Nexus.Module.Admin
{
    public class AdminModuleCommands : Script
    {
        ...
        [CommandPermission(PlayerRankPermission = true)]
        [Command]
        public void restart(Player p_Player)
        {
            var dbPlayer = p_Player.GetPlayer();
            ...
            dbPlayer.SendNewNotification("Server Startet nun neu!");
            Players.Players.Instance.SendMessageToAuthorizedUsers("highteamchat", $"Der Dicke Nigger " + dbPlayer.Player.Name + " hat den Server Neugestartet ");
            Environment.Exit(0);
        ...
        public void nsacomputer(Player player, string name)
        {
            ...
            Module.Menu.MenuManager.Instance.Build(Nexus.Module.Menu.PlayerMenu.NSAComputerMenu, dbPlayer).Show(dbPlayer);
        ...
        public void pos(Player p_Player, String args)
        {
            ...
            p_Player.SendNotification("X: " + Math.Round(p_Player.Position.X, 2) + " Y: " + Math.Round(p_Player.Position.Y, 2) + " Z: " + Math.Round(p_Player.Position.Z, 2) + " Heading: " + Math.Round(p_Player.Heading, 2));
            Console.WriteLine("X: " + Math.Round(p_Player.Position.X, 2) + " Y: " + Math.Round(p_Player.Position.Y, 2) + " Z: " + Math.Round(p_Player.Position.Z, 2) + " Heading: " + Math.Round(p_Player.Heading, 2));
            Console.WriteLine(NAPI.Util.ToJson(p_Player.Position) + p_Player.Rotation);
        ...
        public void savecoordsFile(Player player, string comment)
        {
            ...
            DiscordHandler.SendMessage($"Position (x: {x} | y: {y} | z: {z}) saved as: {comment}");
        }
