...
namespace Nexus.Module.Clothes.Windows
{
            ...
            try
            {
                ...
                if (price > 0)
                {
                    ...
                    else
                    {
                        bool ja = false;
                        if (price < 0) ja = false;
                        if (dbPlayer.money[0] < price) ja = false;
                        if (dbPlayer.money[0] - price > dbPlayer.money[0]) ja = false;
                        dbPlayer.money[0] = dbPlayer.money[0] - price;
                        dbPlayer.Player.TriggerEvent("updateMoney", dbPlayer.money[0]);
                        dbPlayer.Save();
                        ja = true;
