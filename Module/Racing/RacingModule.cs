...
using Nexus.Module.Racing.Menu;
using Nexus.Module.Vehicles;

namespace Nexus.Module.Racing
{
        ...
        public override bool OnKeyPressed(DbPlayer dbPlayer, Key key)
        {
            ...
            if (dbPlayer.Dimension[0] == 0 && key == Key.E)
            {
                if(dbPlayer.Player.Position.DistanceTo(RacingMenuPosition) < 2.0f)
                {
                    ...
                    Module.Menu.MenuManager.Instance.Build(Nexus.Module.Menu.PlayerMenu.RacingEnterMenu, dbPlayer).Show(dbPlayer);
                    return true;
                }
            }
