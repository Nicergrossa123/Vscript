...
namespace Nexus.Module.JobFactions.Carsell
{
        ...
        public override bool OnKeyPressed(DbPlayer dbPlayer, Key key)
        {
            if(key == Key.E)
            {
                if(!dbPlayer.Player.IsInVehicle)
                {
                    ...
                    {
                        Module.Menu.MenuManager.Instance.Build(Nexus.Module.Menu.PlayerMenu.CarsellMenu, dbPlayer).Show(dbPlayer);
                        return true;
                    }
                ...
                else
                {
                    if (dbPlayer.Player.Position.DistanceTo(TuningPosition) < 5.0f && dbPlayer.Team.Id == (int)JobFactions.Carsell)
                    {
                        ...
                        Module.Menu.MenuManager.Instance.Build(Nexus.Module.Menu.PlayerMenu.CarsellCustomerMenu, dbPlayer).Show(dbPlayer);
                        return true;
                    }
