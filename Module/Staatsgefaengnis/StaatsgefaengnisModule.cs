...
using Nexus.Module.Staatsgefaengnis.Menu;
using Nexus.Module.Teams;

namespace Nexus.Module.Staatsgefaengnis
{
    public enum SGJobs
    {
        WASHING = 1,
            ...
            if(key == Key.E && !dbPlayer.Player.IsInVehicle)
            {
                if (dbPlayer.Player.Position.DistanceTo(sgEmergencyPosition) < 2.0f)
                {
                    ...
                    else
                    {
                        ...
                        if (requestSuccess)
                        {
                            Nexus.Module.Service.Service service = new Nexus.Module.Service.Service(dbPlayer.Player.Position, message, (uint)teams.TEAM_MEDIC, dbPlayer, "", telnr);
                            bool status = Nexus.Module.Service.ServiceModule.Instance.Add(dbPlayer, (uint)teams.TEAM_MEDIC, service);
                ...
                if (dbPlayer.Player.Position.DistanceTo(JobMenuPosition) < 1.0f)
                {
                    ...
                    {
                        Module.Menu.MenuManager.Instance.Build(Nexus.Module.Menu.PlayerMenu.SGJobChooseMenu, dbPlayer).Show(dbPlayer);
                        return false;
                    }
