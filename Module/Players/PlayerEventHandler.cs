...
using Nexus.Module.Clothes.Outfits;
using Nexus.Module.Clothes.Props;
using Nexus.Module.Clothes.Team;
...
using Nexus.Module.Injury.InjuryMove;
using Nexus.Module.Items;
using Nexus.Module.Kasino;
using Nexus.Module.Keys;
using Nexus.Module.Keys.Windows;
    ...
    public class PlayerEventHandler : Script
    {
        ...
        public void Keks(Player player, bool state)
        {
            ...
            if (state && !player.IsInVehicle)
            {
                ...
                //iPlayer.Player.TriggerEvent("createphoneobject");
                iPlayer.PlayAnimation((int)(AnimationFlags.Loop | AnimationFlags.OnlyAnimateUpperBody | AnimationFlags.AllowPlayerControl), "anim@cellphone@in_car@ds", "cellphone_text_read_base");
                //NAPI.Player.PlayPlayerAnimation(player, (int)(AnimationFlags.Loop | AnimationFlags.AllowPlayerControl | AnimationFlags.OnlyAnimateUpperBody), "anim@cellphone@in_car@ds", "cellphone_text_read_base", 0.8f);
            }

            if (!state && !player.IsInVehicle)
            {
                iPlayer.StopAnimation();
                iPlayer.Player.TriggerEvent("destroyphoneobject");
            }
