...
using Nexus.Module.Computer.Apps.MarketplaceApp;
using Nexus.Module.Players;
using Nexus.Module.Teams;

namespace Nexus.Module.Service
{
        ...
        public void addServiceRequest(Player Player, string department, string message, bool number)
        {
            ...
            switch (department)
            {
                case "police":
                    ...
                    if (dbPlayer.IsACop() && dbPlayer.IsInDuty() && dbPlayer.TeamId != (int)teams.TEAM_POLICE)
                    {
                        return;
                    }
